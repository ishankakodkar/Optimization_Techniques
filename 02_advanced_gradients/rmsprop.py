...
Algorithm:
E[g^2]_t = ρ E[g^2]_{t-1} + (1-ρ) (∇f(x_t))^2
x_{t+1} = x_t - α / (sqrt(E[g^2]_t) + ε) * ∇f(x_t)
...
class RMSpropOptimizer:
    ...
    """
    def __init__(self, learning_rate: float = 0.001, decay_rate: float = 0.9, epsilon: float = 1e-8, max_iterations: int = 1000, tolerance: float = 1e-6, verbose: bool = False):
    ...
    def minimize(self, objective: Callable, initial_point: np.ndarray, gradient: Optional[Callable] = None) -> Tuple[np.ndarray, dict]:
        ...
        if self.verbose:
            print(f"Starting RMSprop optimization")
            print(f"Learning rate: {self.learning_rate}, Decay rate: {self.decay_rate}, Epsilon: {self.epsilon}")
    ...
    def plot_convergence(self, title: str = "RMSprop Convergence"):
        ...
        ax3.set_title('Learning Rate (Mean)')
