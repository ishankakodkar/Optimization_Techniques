...
Algorithm:
g_{t+1} = g_t + (∇f(x_t))^2
x_{t+1} = x_t - α / (sqrt(g_{t+1}) + ε) * ∇f(x_t)
...
class AdaGradOptimizer:
    ...
    """
    def __init__(self, learning_rate: float = 0.1, epsilon: float = 1e-8, max_iterations: int = 1000, tolerance: float = 1e-6, verbose: bool = False):
    ...
    def minimize(self, objective: Callable, initial_point: np.ndarray, gradient: Optional[Callable] = None) -> Tuple[np.ndarray, dict]:
        ...
        grad_squared = np.zeros_like(x)
        self.history = {key: [] for key in self.history.keys()}
        if self.verbose:
            print(f"Starting AdaGrad optimization")
            print(f"Learning rate: {self.learning_rate}, Epsilon: {self.epsilon}")
    ...
    def plot_convergence(self, title: str = "AdaGrad Convergence"):
