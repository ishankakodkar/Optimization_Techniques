...
- ∇f(x_k): gradient of function f at x_k

Convergence:
- For convex functions, GD converges to the global minimum.
- The convergence rate is O(1/k) for constant step size.
- Step size α must be chosen carefully: too large may diverge, too small may slow convergence.

Applications:
- Machine learning model training (linear regression, logistic regression)
- Neural networks (as a base for more advanced optimizers)
...
class GradientDescent:
    ...
    """
    def __init__(self, learning_rate: float = 0.01, max_iterations: int = 1000, tolerance: float = 1e-6, verbose: bool = False):
    ...
    def plot_convergence(self, title: str = "Gradient Descent Convergence"):
